...
class DinoEncoder(AbstractEmbModel):
    def __init__(
        ...
        self.cond_masked_tokens = nn.Parameter(
            torch.randn(self.num_condition_tokens, 1024),
            requires_grad=True,
        )
        self.cond_pos_emb = posemb_sincos_1d(self.num_pos_emb_tokens, 1024)
    ...
    def forward(
        ...
        grouped_tokens = None
        cond_feats = (
            self.get_features(pixel_values)
        ...
        cond_feats = self.group_tokens(cond_feats)
        if return_grouped_tokens:
            grouped_tokens = cond_feats.clone()
        patches_to_pick, patches_xy = self.get_patches_to_select(
            cond_feats
        )

        cond_feats = self.cond_projection(cond_feats)
        cond_feats = cond_feats.view(
            b, l, h // 2, w // 2, self.proj_channels * 4
        )
